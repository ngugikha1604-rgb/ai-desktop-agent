...
| Browser | Mở URL, tìm kiếm Google, điều khiển tab, **tìm kiếm & đọc kết quả web** |
...
| `search_web`, `open_url`, `web_search` | ✅ SAFE | — |
...
│
├── tools/
│   ├── registry.py        # ToolSpec + TOOL_REGISTRY (17 tools)
│   ├── web_search.py      # DuckDuckGo search — trả về text để model đọc (NEW)
│   ├── __init__.py
│   └── ... (17 tool files)
│
├── ui/
...
| Web search | duckduckgo-search (DuckDuckGo API, no API key) |
...
- [x] **Web Search: DuckDuckGo — model đọc được kết quả thực từ internet (không cần API key)**
