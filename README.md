...
| Browser | Mở URL, tìm kiếm Google, điều khiển tab, tìm kiếm & đọc kết quả web, **thời tiết trực tiếp** |
...
│
├── tools/
│   ├── registry.py        # ToolSpec + TOOL_REGISTRY (19 tools)
│   ├── web_search.py      # DuckDuckGo search — trả về text để model đọc
│   ├── web_read.py        # Đọc nội dung URL (direct + Jina AI fallback)
│   ├── get_weather.py     # Thời tiết trực tiếp từ wttr.in API
│   ├── __init__.py
│   └── ... (19 tool files)
│
├── ui/
 ...
 AgentWorker (QThread, timeout 300s)
   ...
   Agent.run()
    ...
    └─► Agent Loop (max 10 bước)
         ...
         │    tools=[JSON schema × 19] → model trả tool_calls
...
|------------|-----------|
| LLM | Ollama — Qwen2.5 3B (planner, native tool calling) / Qwen2.5 0.5B (analyzer, response) |
| STT | faster-whisper `base` (local, vi + en) |
| UI | PySide6 (Qt6) |
| Database | SQLite |
| Validation | Pydantic v2 |
| Web search | ddgs (DuckDuckGo, no API key) |
| Web read | requests + beautifulsoup4 + Jina AI Reader (JS fallback) |
| Weather API | wttr.in (no API key) |
...
- [x] **Web Search: DuckDuckGo (ddgs) — model đọc kết quả thực từ internet**
- [x] **Web Read: đọc nội dung URL với Jina AI fallback cho JS-rendered sites**
- [x] **Weather: wttr.in API trực tiếp — không cần web_search chain**
- [x] **Native Tool Calling: migrate sang Ollama tools API — bỏ prompt-based JSON, system prompt từ ~650 → ~150 token**
